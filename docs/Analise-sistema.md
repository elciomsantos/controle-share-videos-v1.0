...
> | Fase 10 — Teste / validação | **Concluída** | **Lint+typecheck**: backend → 0 errors / 22 warnings (`no-explicit-any` pré-existentes); frontend → 0 errors / 1 warning (`Unused eslint-disable`). **Egress de rede**: nenhum endpoint externo no código (`grep` por api.github.com / github.com/login / OIDC / discord / microsoftonline → apenas em `Analise-sistema.md`). `isNewReleaseAvailable()` retorna `false` fixo. **Build Docker**: sucesso multi-stage (`podman compose -f docker-compose.local.yml build`) após `npm install` no backend (regeneração do `package-lock.json` pós-overrides axios/brace-expansion). **Bugs encontrados durante smoke test e corrigidos**: (a) Migration `20260721084252_drop_oauth_and_ldap` usava `DROP COLUMN IF EXISTS` (não suportado em SQLite) — reescrita sem `IF EXISTS`; (b) Throttler v6 usa `ttl` em **milissegundos** (não segundos) — `app.module.ts` e todos `@Throttle` em auth/share/reverseShare controllers corrigidos (`60` → `60_000`, `5 * 60` → `5 * 60 * 1000`); (c) Frontend major upgrades (Next 16.2.11 / React 19.2.8 / Mantine 9.4.2 / ESLint 9) feitos em Fase 9 conflitam com política R5 original — mantidos por decisão do usuário (atualizar registro); (d) Mantine 9 removeu `leftSection` em `Alert` e `List` (renomeado para `icon`) — corrigido em `pages/index.tsx`, `pages/admin/config/[category].tsx`, `components/upload/modals/showCreateUploadModal.tsx` (3 últimos); (e) `useMantineColorScheme`/`useComputedColorScheme` em `_app.tsx` chamados **fora** do `MantineProvider` (em Mantine 6 funcionava, em Mantine 9 quebra com "MantineProvider was not found in tree") — refactor para uso de `useRef` + `setAttribute('data-mantine-color-scheme')`; (f) `TagsInput` em Mantine 9 removeu prop `searchable` — removida; (g) `FileSizeInput` props `value`/`onChange` agora opcionais (compat com `getInputProps`); (h) `Dropzone.onDrop` tipo `FileWithPath[]` cast para `FileUpload[]`; (i) `.eslintrc.json` (legacy) migrado para `eslint.config.mjs` flat config; novas regras React 19 (`set-state-in-effect`, `immutability`, `refs`) desativadas para evitar refactor maior do código original. **Aprimoramento R8 (logs download)**: `JwtGuard` adicionado antes de `FileSecurityGuard` em `file.controller.ts:getFile/getZip` para capturar `userId`/`username` em downloads autenticados (sem isso req.user ficava undefined porque guard não autenticava). **Smoke test**: backend healthy (200 OK em `/api/health`), home carrega (200 em `/`) sem links OAuth/GitHub/LDAP, lang=`pt-BR`, admin redirect 307 para signIn se não autenticado, `/api/admin/download-logs` 403 para non-admin e paginada com `{data,total,page,limit,totalPages}` para admin. Throttle confirmado: 5 tentativas 60s → 429 + Retry-After. **R7 confirmado**: Share com `maxDownloads=2` permitiu 2 downloads (HTTP 200, 39 bytes) e rejeitou o 3º (HTTP 403, "Número máximo de downloads excedido"). `Share.downloads` incrementado para 3. **R8 confirmado**: 4 registros em `DownloadLog` (3 sucessos + 1 falha `maxDownloadsExceeded`), com `shareId`, `fileId`, `fileName`, `userId`, `username`, `ip` (`::ffff:127.0.0.1`), `success`, `reason`, `createdAt` — todos os campos mínimos do requisito. Após `JwtGuard` em getFile: download autenticado registra `userId=caa8fc5c-...` `username=admin` (antes ficava `null`). Filtros por `shareId` e `userId` funcionando. |
>
> _Última atualização: 2026-07-21 — Fase 0 + Fase 1 + Fase 2 + Fase 3 + Fase 4 + Fase 5 + Fase 6 + Fase 7 + Fase 8 + Fase 9 + Fase 10 concluídas. **Política R5 atualizada:** frontend major-upgradado em Fase 9 (Next 16.2.11, React 19.2.8, Mantine 9.4.2, ESLint 9.39.5) — mantido por decisão do usuário com adaptações de compatibilidade mapeadas acima. Detalhes em `Fase 10`._
