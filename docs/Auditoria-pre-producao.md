...
| **GAP-06** | 🟢 P3 | Criados: `scripts/monitoring/grafana-dashboards/overview.json` (dashboard com painéis: UP targets, CPU, memória, disco, HTTP 5xx, latência p99, integrity check do SQLite, scrape failures), `scripts/monitoring/grafana-dashboards/dashboards.yml` (provisionamento), `scripts/monitoring/alerts.yml` (9 regras: BackendDown, HighCpu, HighMemory, DiskSpaceLow/Critical, High5xxRate, HighLatencyP99, SqliteIntegrityFailure). `prometheus.yml` atualizado com `rule_files`. `docker-compose.monitoring.yml` monta `alerts.yml` e provisiona o secret `grafana_admin_password` via bind. Script `scripts/provision/grafana-secret.sh` gera senha forte (32 bytes base64url) e escreve em `scripts/secrets/` (gitignorado). Em 2026-08-09 o stack foi corrigido: backend passa a exportar métricas próprias via prom-client em `GET /api/metrics` (o scrape anterior apontava para `/api/health`, que não é formato Prometheus válido); o job/alerta `CaddyDown` foi removido porque as métricas do Caddy exigiriam habilitar a admin API (`admin off` por hardening); 5xx/latência agora vêm das métricas do backend e o Prometheus foi anexado à rede do app. |
...
7. ✅ **Monitoramento** — Prometheus + Grafana + Loki + Promtail + dashboard provisionado (`overview.json`) + `alerts.yml` (9 regras) + script de criação de secret do Grafana; backend exporta métricas próprias em `/api/metrics`
