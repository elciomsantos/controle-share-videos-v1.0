...
3. In the web editor, paste the content of the [docker-compose]
...
```bash
git clonehttps://github.com/GhostTerminals/Controle-share-videos-v1.0.git
cd controle-share-videos
...
npm install
npm run build
pm2 start --name="controle-share-videos-v1.0-backend" npm -- run prod
...
pm2 start npm --name "controle-share-videos-v1.0" -- run start
```

**Uploading Large Files**: By default, controle-share-videos-v1.0  uses a built-in reverse proxy to reduce the installation steps. However, this reverse proxy is not optimized for uploading large files. If you wish to upload larger files, you can either use the Docker installation or set up your own reverse proxy. An example configuration for Caddy can be found in `./reverse-proxy/Caddyfile`.
